        ...
        <div v-if="show" class="modal-mask">
            <div class="confirmation-modal">
                ...
                <div class="modal-content">
                    <span v-if="loading" class="sending"> Enviando... </span>

                    <div v-else class="sended">
                        <span v-if="success" class="success"> Enviado com sucesso! </span>

                        <span v-else class="failed"> Falha ao enviar. </span>
                    </div>

                    <div
                        ref="success-mark"
                        class="circle-loader"
                        :class="{ 'load-complete': loaded }"
                    >
                ...
                <div class="modal-footer">
                    <div v-if="!loading" class="modal-action-buttons">
                        <button v-if="success" class="close" @click="close">Finalizar</button>

                        <button v-else class="retry" @click="send">Tentar novamente</button>
                    </div>
                </div>
            </div>
        </div>
...
function send() {
    ...
    emailjs
        .send('service_0fsg59b', 'template_qxr7k6k', props.params.value, {
